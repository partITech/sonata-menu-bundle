...
class MenuItemAdmin extends AbstractAdmin
{
    ...
    protected function configureFormFields(FormMapper $formMapper): void
    {
        ...
        if ($this->container->hasParameter('sonata.page.page.class')) {
            ...
            $menuClass = $this->container->getParameter('sonata_menu.entity.menu');

            $em = $this->getModelManager()->getEntityManager($menuClass);
            $builder = $em->createQueryBuilder('m');
            $query = $builder->select('m')
                ->from($menuClass, 'm')
                ->where('m.id = '.$menu->getId())
                ->getQuery();
            ...
            $query = $builder->select('p.id, p.name, p.url')
                       ->from($pageClass, 'p')
                        ->where('p.site ='.$menu->getSite()->getId())
                       ->getQuery();
